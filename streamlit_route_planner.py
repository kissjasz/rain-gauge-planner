...
# =======================================================
        ...
        if not df_filtered.empty or include_base:
            ...
            if map_obj:
                ...
                if map_mode == 'select':
                    st.info("🎯 **โหมดเลือกสถานี**: คลิก marker เพื่อเปิดข้อมูล กดปุ่มยืนยันเพิ่ม/ยกเลิก ด้านล่างของแผนที่")
                else:
        ...
        if 'route_result' in st.session_state:
            ...
            if ordered_stations and path_coords:
                ...
                if len(path_coords) >= 2:
                    ...
                    try:
                        coords = "/".join([f"{c[0]},{c[1]}" for c in path_coords])
                        maps_url = f"https://www.google.com/maps/dir/?api=1&waypoints={coords}"
